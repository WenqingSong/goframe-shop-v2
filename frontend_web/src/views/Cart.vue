...
const changeGoodsSum = async ({ goods_options_id, id, count }) => {
    const res = await editCart({ goods_options_id, id, count });
    console.log('res:', res);
    if (res.code === 0) {
        console.log(`${id} ${goods_options_id} change count ${count}`);
    }
};
...
const onSubmit = () => {
    const goods = cartStore.data
        .filter?.(it => it.checked)
        .map(it => ({
            goods_id: it.goods_options_info.goods_id,
            count: it.count,
            cart_id: it.id,
            goods_options_info: it.goods_options_info,
            goods_info: it.goods_options_info.goods_info,
        }));
    if (goods.length > 0) {
        // console.log(goods);
...
<template>
    <main class="container">
        ...
        <main v-if="isLogin" class="content">
            <van-list class="goods_list" v-if="cartStore.data?.length > 0" finished-text="没有更多了" offset>
                <van-swipe-cell class="goods_item" v-for="it in cartStore.data">
                    ...
                    <template #right>
                        <van-button square text="删除" type="danger" class="delete-button" @click="deleteGoods(it.id)" />
                    </template>
                </van-swipe-cell>
            </van-list>
        ...
        <div v-if="!isLogin" class="no_login_content">
            <van-empty class="thumbnail" description="未登录">
                <van-button type="primary" class="login_btn" @click="$router.push('/login')">登录</van-button>
            </van-empty>
        </div>
        <van-submit-bar v-if="isLogin && cartStore.data?.length > 0" class="submit" :price="totalPrice"
